...
export async function proxy(request: NextRequest) {
  ...
  // Si l'admin est connecté et navigue sur l'espace client (hors ?client=1), le ramener automatiquement sur l'espace Admin '/admin'
  if (isAuthenticated && !request.nextUrl.pathname.startsWith('/admin') && !request.nextUrl.pathname.startsWith('/api') && !request.nextUrl.searchParams.has('client')) {
